...
- `mauro-files/run*.sh` launchers share one skeleton, `mauro-files/scripts/run-common.sh` (env detection via `WSL_DISTRO_NAME`/`uname`, `.venv` check, Triton pre-warm, the final per-environment `python main.py` invocation). Each `run-*.sh` is a thin wrapper: it sets an `EXTRA_ARGS` bash array (plus any extra env vars) and sources the common script — never edit the detection/pre-warm logic in more than one place again, it only lives in `run-common.sh` now.
  - `mauro-files/run.sh` — baseline, `EXTRA_ARGS=()`. Runs `python main.py --disable-pinned-memory --use-sage-attention` on WSL (see per-environment variants below).
  - `mauro-files/run-minimax.sh` — adds `--reserve-vram 2`, for `video/minimax_h3_i2v_wsl.json` on the 16GB card (renamed from `run-reserve-vram.sh`).
  - `mauro-files/run-qwen-img-edit.sh` — tuned for `image/qwen_image_edit_2509.json` on the 16GB card: adds `--reserve-vram 2`, `--lowvram`, `--disable-cuda-malloc`, and exports `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`. Needed because the fp8 diffusion model alone (~19GB on disk) is bigger than the card's entire 16GB VRAM budget. Note: ComfyUI defaults to the `cudaMallocAsync` CUDA backend, under which `expandable_segments:True` is silently a no-op — `--disable-cuda-malloc` is required to force the native PyTorch allocator that actually honors it; without it the OOM reproduces identically regardless of the other flags. Pairs with setting the workflow's `CLIPLoader` `device` widget to `"cpu"` (already done), which keeps the ~8GB text encoder off VRAM entirely.
